  ...
  override def parse(m: Message, b: BotCommand, r: ServerResponder): Unit = {
    ...
    if(m.command == MessageCommands.INVITE){
      if(m.sender.isAdmin){ //cooldown <= System.currentTimeMillis()/1000
        ...
        r.notice(m.sender.nickname, "Invites are currently disabled")//s"I am currently on invite cooldown. Please wait another $timeleft seconds and invite me again")
      }
    }
  }
}
